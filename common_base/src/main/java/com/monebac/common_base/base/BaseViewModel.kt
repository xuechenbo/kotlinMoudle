    ...
    private fun onError(e: Exception, showErrorToast: Boolean) {
        when (e) {
            is ApiException -> {
                when (e.code) {
                    // 其他api错误
                    -1 -> if (showErrorToast) BaseApplication.application.showToast(e.message)
                    // 其他错误
                    else -> if (showErrorToast) BaseApplication.application.showToast(e.message)
                }
            }
            // 网络请求失败
            is ConnectException, is SocketTimeoutException, is UnknownHostException, is HttpException ->
                if (showErrorToast) BaseApplication.application.showToast("网络请求失败")
            // 数据解析错误
            is JSONException ->
                if (showErrorToast) BaseApplication.application.showToast("数据解析错误")
            // 其他错误
            else ->
                if (showErrorToast) BaseApplication.application.showToast(e.message ?: return)
        }
    }
}